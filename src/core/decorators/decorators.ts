...
import { Entity, EntityRepository, loadWithInstance } from '../entity';
import {
  EntityEvent,
  EventBus,
...
export function onEvent(target: any, propertyKey: string | symbol) {
  index[target.constructor.name] = index[target.constructor.name] || {};
  ...
  while (superclass.name !== 'Object') {
    ...
    superclassCstor = Object.getPrototypeOf(
      superclassCstor.prototype
    ).constructor;
...
export class DecoratedEntityRepository implements EntityRepository {
  ...
  public load(
    construct: { new (...arg: any[]) },
    id: string,
    ...additionalConstArgs: any[]
  ): Promise<any> {
    const delegate: any = new construct(id, ...additionalConstArgs);
    if (index[construct.name]) {
      const onEventMetadata: OnEventMetadata = Reflect.getMetadata(
        METADATA_ON_EVENT,
        index[construct.name]
      );
...
export class DecoratedEventBus implements EventBus {
  ...
  public subscribe(
    listener: (event: EntityEvent, isReplaying?: boolean) => void,
