import { defaultEsContext, Dispatcher, EntityEvent, User, onEvent } from './';
...
describe('es-main', () => {
  it('loads entities', async () => {
    ...
    const entity: EsTestEntity = await entityRepository.load(
      EsTestEntity,
      '123'
    );
    entity.doSomething();
    ...
    await delay(100);
    const again: EsTestEntity = await entityRepository.load(
      EsTestEntity,
      '123'
    );
    expect(again.lastEvent.name).toBe('EsEvent');
    const another: EsTestEntity = await entityRepository.load(
      EsTestEntity,
      '223'
    );
  ...
  it('can seed events before', async () => {
    let { entityRepository } = defaultEsContext.withEvents({
      '423': [new EsEvent('seeded1')],
    });
    const entity: EsTestEntity = await entityRepository.load(
      EsTestEntity,
      '423'
  ...
  it('can seed events after', async () => {
    // Store an event
    let { entityRepository } = defaultEsContext.atTime(1000);
    ...
    entityRepository = defaultEsContext.withEvents({
      '523': [
        new EsEvent('seeded-in-future-2', 1002),
    ...
    expect((entity.receivedEvents[0] as EsEvent).description).toBe(
      'seeded-in-past'
    );
    expect((entity.receivedEvents[1] as EsEvent).description).toBe(
      'seeded-same'
    );
    expect((entity.receivedEvents[2] as EsEvent).description).toBe(
      'didSomething'
    );
    expect((entity.receivedEvents[3] as EsEvent).description).toBe(
      'seeded-in-future'
    );
    expect((entity.lastEvent as EsEvent).description).toBe(
      'seeded-in-future-2'
    );
  });
  it('does not mutate modifiers of the parent context', async () => {
    let { eventDispatcher: er1, eventStore: es1 } =
      defaultEsContext.atTime(1000);
